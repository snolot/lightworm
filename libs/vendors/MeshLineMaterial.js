...
function MeshLineMaterial( parameters, vs, fs ) {

	var vertexShaderSource = vs || [
...
'	 }',
'    gl_FragColor = c;',
// '	 gl_FragColor.rgb *= smoothstep( depth.x, depth.y, vZ );',
		'float alpha = smoothstep( depth.x, depth.y, vZ );',
	'gl_FragColor.rgb *= mix(gl_FragColor.rgb*.25, gl_FragColor.rgb, alpha );',
// '	 gl_FragColor.rgb *= ( vZ * .1 );',
// '	 gl_FragColor.rgb *= mix( vec3( 1. ),gl_FragColor.rgb,  ( vZ * .1 ) );',
'	 gl_FragColor.a *= step(visibility.x, vCounters) * step(vCounters,visibility.y) * ( .5 + alpha );',
'}' ].join( '\r\n' );
	...
	this.visibility = check( parameters.visibility, new THREE.Vector2( 0,1 ) );
	this.depth = check( parameters.depth, new THREE.Vector2( 0,1 ) );
	...
	var material = new THREE.RawShaderMaterial( {
		uniforms:{
			...
			visibility: {type: 'v2', value: this.visibility},
			depth: {type: 'v2', value: this.depth},
